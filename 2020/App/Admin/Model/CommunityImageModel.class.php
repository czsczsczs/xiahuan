<?php
namespace Admin\Model;

use Think\Model;

/**
 * 
 *[社區圖片模型]
 * @author echo@poptek.cn
 * @dateTime 2018/09/06
 * @return [type] [description]
 *
 */
class CommunityImageModel extends Model
{

    /**
     * 自动验证
     */
    protected $_validate = array(
        // array('title','require','標題必須填寫',1),
        
    );


    /**
     * 
     *自动完成
     *
     */
    protected $_auto = array ( 
        array('created','time',1,'function'),
        array('updated','time',3,'function'),
     );

	public function communityImageList($where,$firstRow = '',$listRows = '',$keyword = ''){
		$Data = self::where($where)
			->limit($firstRow.','.$listRows)
            ->order('created desc')
            ->select();
        return $Data;
	}


	public function communityImageCount($where){
		$count = self::where($where)
            ->order('created desc')
            ->count();
        return $count;
	}



	public function communityImageCreate($data){
		if($data['recommend']){
			$data['re_time'] = time();
		}
		/*自動驗證*/
		$return = self::create($data);

		if(!$return){
			return false;
		}else{
			return self::add($return);
		}
	}


	public function communityImageUpdate($id,$data){
		if($data['recommend']){
			$data['re_time'] = time();
		}
		/*自動驗證*/
		$return = self::create($data,2);

		if(!$return){
			return false;
		}else{
			return self::where('id='.$id)->save($return);
		}
	}





}


?>