...
class ApiController extends BaseController 
{
    ...
    public function index()
    {
        ...
        foreach ($activity as $key1 => $value1) {
            $activity[$key1]['cover'] = 'http://macauonline.net/xiahuan/2020'.$value1['cover_width_300'];
        ...
        foreach ($shop as $key2 => $value2) {
            $shop[$key2]['cover'] = 'http://macauonline.net/xiahuan/2020'.$value2['cover_width_300'];
        ...
        foreach ($post as $key3 => $value3) {
            $post[$key3]['cover'] = 'http://macauonline.net/xiahuan/2020'.$value3['cover_width_300'];
    ...
    public function lists($type)
    {
        ...
        if($type == 'activity'){
            ...
            ->order('created desc')
            ->select();
            foreach ($street as $skey => $svalue) {
                $street[$skey]['type'] = 'street';

            }
            $lists = array_merge($post,$street);
        }elseif($type == 'culture'){
        ...
        foreach ($lists as $key => $value) {
            $lists[$key]['cover'] = 'http://macauonline.net/xiahuan/2020'.$value['cover_width_300'];
...
// </p>
           ...
           $data = $this->postModel->where($where)
            ...
            ->find();
            $data['cover'] = 'http://macauonline.net/xiahuan/2020'.$data['cover_width_300'];
        }elseif ($type == 'news') {
        ...
        $data['content'] = str_replace('src="/','src="http://macauonline.net/xiahuan/2020/',htmlspecialchars_decode($data['content']));
        ...
        if($data['images']){
            ...
            foreach ($imagesId as $key => $value) {
                ...
                $data['image'][$key]['savepath'] = 'http://macauonline.net/xiahuan/2020/'.$file['save_path'];
            }
        }
    ...
    public function banner($id){
        ...
        $data = $this->slideModel->where($where)
            ->find();
        $data['image'] = 'http://macauonline.net/xiahuan/2020'.$data['image'];
        return $this->ajaxReturn(success($data));
    }
