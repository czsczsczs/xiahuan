...
class PCbannerModel extends Model
{
	...
	public function pcbannerList($where,$firstRow = '',$listRows = '',$keyword = ''){
	...
	public function pcbannerCount($where){
	...
	public function pcbannerCreate($data){
		if($data['recommend']){
			$data['re_time'] = time();
		}
		/*自動驗證*/
	...
	public function pcbannerUpdate($id,$data){
		if($data['recommend']){
			$data['re_time'] = time();
		}
		/*自動驗證*/
